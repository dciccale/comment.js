 ...
 * Parses comments from the given string and generates two HTML sources.
 ...
 o }
\*/
module.exports = function (content, filename, sourceFileName) {
  var docsTitle = content.match(/^[^"]*"([^"]+)"/),
    ...
    srcTemplate = fs.readFileSync(path.resolve(path.normalize(__dirname), 'template', 'template-src.html'), 'utf-8');
    ...
    level.sort().forEach(function (_level) {
      ...
      if (itemData.line) {
        chunk += '<span class="cjs-sourceline">Defined in: ' +
          ...
          ' in the source" href="' + srcfilename + '#L' + itemData.line + '">' +
  ...
  out.source = handlebars.compile(srcTemplate)({title: path.basename(filename), src: src});
