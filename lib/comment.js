...
function commentjs(files, options) {
  ...
  if (!files.length || /''/.test(files[0]) || !files[0]) {
    ...
    console.log('\n   -o    set the output directory for the docs files\n');
    process.exit(1);
  }
  ...
  function pushFiles(file) {
    ...
    // normalize file path
    file = path.normalize(path.join(sourceDir, filePath));

    if ((!fileRegex || fileRegex.test(filename)) && utils.file.exists(file) && !utils.file.isDir(file)) {
      files.push(file);
