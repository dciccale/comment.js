...
function commentjs(files) {
  ...
  function createRelativeSrcFileName(file) {
    return srcFolder + path.sep + changeFileExt(file, '-src.html');
  }
  ...
  if (files.length === 1 && path.extname(files[0]) === ".json") {
    ...
    trackingID = options.ga_trackingID || trackingID;
  }
  ...
  function generateDocs() {
    ...
    files.forEach(function (file, i) {
      ...
      var code = readFileSync(file).replace(/\r\n|\r/g, '\n'),
        relativeSourceFileName = createRelativeSrcFileName(file),
        // link to source file
