 ...
 * Parses comments from the given string and generates two HTML sources.
 ...
 o }
\*/
module.exports = function (content, filename, sourceFileName, generateSource) {
  ...
  if (generateSource) {
    srcTemplate = utils.file.read(path.resolve(path.normalize(__dirname), 'template', 'template-src.html'), 'utf-8'),
    compiledSrcTemplate = handlebars.compile(srcTemplate);
  }
